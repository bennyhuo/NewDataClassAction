...
public class CreateFileFromTemplateDialog extends DialogWrapper {
    ...
    protected CreateFileFromTemplateDialog(@NotNull Project project) {
        ...
        formatJson.addActionListener(new AbstractAction() {
            @Override
        ...
        dataSourceInput.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                //在最后一个字符换行，可触发 ok 操作
                if (e.getKeyCode() == KeyEvent.VK_ENTER
                        && dataSourceInput.getCaretPosition() == dataSourceInput.getText().length()) {
                    doOKAction();
                } else {
                    super.keyReleased(e);
                }
    ...
    private boolean formatJson() {
        try {
            String json = dataSourceInput.getText();
            json = json.trim();
            if (json.startsWith("{")) {
                JSONObject jsonObject = new JSONObject(json);
                String formatJson = jsonObject.toString(4);
                dataSourceInput.setText(formatJson);
            ...
            Toast.make(project, dataSourceInput, MessageType.ERROR, "Invalid Json Text:" + e.getMessage());
        }
        return false;
    }
        ...
        public <T extends PsiElement> T show(@NotNull String errorTitle, @Nullable String selectedTemplateName,
            ...
            myDialog.myCreator = new ElementCreator(myProject, errorTitle) {
                ...
                protected PsiElement[] create(String newName) throws Exception {
                    ...
                    String json = myDialog.dataSourceInput.getText();
                    KtFile ktFile = (KtFile) element;
                    KtClass ktClass = (KtClass) ((KtLightClass) ktFile.getClasses()[0]).getKotlinOrigin();
                    new JsonParser(json, ktClass).parse();
                    created.set(element);
                    if (element != null) {
